...
from dataclasses import replace; from pathlib import Path
...
model = DarwinXModel(cog); torch.set_default_dtype(prev)
...
model.eval(); model.to(dtype=torch.bfloat16); model.enable_dual_gpu(gpu0=0, gpu1=1)
...
FACTS = [("arquivo-baleia","457291"),("chave-golfinho","185034"),("documento-falcao","628410")]
for entity, code in FACTS:
    ...
    memory.teach(q_out.hidden_states, a_out.hidden_states, event_type="explicit_teaching", provenance="human", tags=(entity,))
...
for entity, code in FACTS:
    for pi, p in enumerate(PARAS):
        ...
        recalls = memory.recall(out.hidden_states, top_k=3, require_verified=True)
        r1 = recalls[0]
        r2 = recalls[1] if len(recalls) > 1 else None
        margin = r1.score - r2.score if r2 else 999.0
        r1_tag = r1.record.tags[0] if r1.record.tags else "?"
        r2_tag = r2.record.tags[0] if r2 and r2.record.tags else "?"
        hit = entity in r1.record.tags if r1.record.tags else False
        kind = "LIT" if pi == 0 else "PARA"
        print(f"  [{kind}] {entity}: best={r1.score:.3f}({r1_tag}) 2nd={r2.score:.3f}({r2_tag}) margin={margin:.3f} {'HIT' if hit else 'MISS'}")
